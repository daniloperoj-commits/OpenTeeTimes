...
def pintar_editor_cache_distancias():
    ...
    with col_edit_1:
        nueva_distancia = st.number_input(
            ...
            step=0.1,
            format="%.1f",
            key="cache_distancias_nueva_distancia"
        )
    with col_edit_2:
        nueva_duracion = st.number_input(
            ...
            step=1,
            key="cache_distancias_nueva_duracion"
        )
    ...
    with col_btn_1:
        if st.button("Guardar cambios caché", key="guardar_cambios_cache_distancias"):
            ...
            cache["distancias"][clave_seleccionada]["fecha_modificacion_manual"] = (
                datetime.now(TZ).isoformat(timespec="seconds") if "TZ" in globals() else datetime.now().isoformat(timespec="seconds")
            )
