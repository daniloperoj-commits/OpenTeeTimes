...
def consultar_campo_golfmanager(campo, fecha, hora_inicio, hora_fin, jugadores, filtro_hoyos, filtro_tipo):
    ...
    # En Golfmanager el endpoint ya pertenece a un campo concreto.
    # Si solo hay un recorrido válido, usamos ese recorrido para construir el resultado.
    recorrido = recorridos_validos[0]

    fecha_golfmanager = str(fecha).replace("/", "-")

    params = {
        "idResourceType": campo.get("idResourceType", campo.get("resourcetype", 1)),
        "start": f"{fecha_golfmanager}T{hora_inicio}:00",
        "cachebreaker": int(datetime.now().timestamp() * 1000)
    }

    id_resource = campo.get("idResource", campo.get("resource"))
