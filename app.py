...
def calcular_distancias_ruta_heigit(lat_ref, lon_ref, campos, radio_km):
    """
    Calcula distancia en ruta para campos que ya han pasado Bounding Box + Haversine.

    Usa HeiGIT/OpenRouteService Matrix API en modo 1 origen -> N destinos.
    ...
    for i, campo in enumerate(campos_validos):
        ...
        if distancia_ruta_km <= radio_km_float:
            matrix_txt = f"OK - {distancia_ruta_km:.1f} km ruta".replace('.', ',')
            resultado_txt = "En rango"
            campos_en_rango_ruta.append(campo)
        else:
            matrix_txt = f"KO - {distancia_ruta_km:.1f} km ruta".replace('.', ',')
